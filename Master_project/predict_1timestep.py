...
def pre_process():
    ...
    stock_data.drop(columns=['change1', 'change5', 'change10', 'change20'], axis=1, inplace=True)
...
def fit_model(m):  # m = parameters
    ...
    model.add(LSTM(m, input_shape=(train_X.shape[1], train_X.shape[2]),
    ...
    # fit network
    model.fit(train_X, train_Y, epochs=50, batch_size=20, validation_split=0.2, verbose=0, shuffle=False,
              callbacks=[EarlyStopping(monitor='val_loss', min_delta=0.002, patience=10, verbose=0, mode='min')])
...
def model_lstm():  # to predict next n-day change
    ...
    model.compile(loss='mae', optimizer='adam')
    # fit network
    history_lstm = model.fit(train_X, train_Y, epochs=120, batch_size=20, validation_split=0.2, verbose=0,
...
def model_gru():  # to predict next n-day change
    ...
    history_gru = model.fit(train_X, train_Y, epochs=25, batch_size=20, validation_split=0.2,
...
def model_mixed():
    train_X, test_X, train_Y, test_Y = set_data()
    model = Sequential()
    model.add(GRU(40, return_sequences=True, input_shape=(train_X.shape[1], train_X.shape[2])))
                  # kernel_regularizer=keras.regularizers.l2(0.01)))
    model.add(Dropout(0.3))
    # model.add(keras.layers.BatchNormalization())
    ...
    model.compile(loss='mae', optimizer='adam')
    # fit network
    history_mixed = model.fit(train_X, train_Y, epochs=60, batch_size=20, validation_split=0.2, verbose=0,
                              shuffle=False, callbacks=[EarlyStopping(monitor='val_loss', min_delta=0.002,
                                                                      patience=12, verbose=0, mode='min')])
...
def predict_result(model_name: str):  # import model_name
    ...
    test_true = concatenate((test_X[:, : 6], test_Y), axis=1)
    test_true = scaler_fit.inverse_transform(test_true)
