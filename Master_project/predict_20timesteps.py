...
def pre_process():
    # stock_data = get_latest_data('0000934')
...
def reframed_data():
    data = pre_process()
    data.drop(columns=['change20'], axis=1, inplace=True)
    values = data.values
    data = values.astype(float32)
    reframed = series_to_supervised(data, 1, 20)
    for i in range(20):
        for j in range(5):
            col_index: int = 6 + i
            reframed.drop(reframed.columns[[col_index]], axis=1, inplace=True)  # shape = (3337, 26)
    return reframed
...
def set_data():
    train, test = normalize_data()      # finish reframing and normalization

    # split train and test data
    train_X, train_Y = train[: , : 6], train[: , 6: ]       # y.shape(3337, 20)
...
def model_lstm():  # to predict next n-day change
    ...
    model.compile(loss='mae', optimizer='adam')
    # fit network
    history_lstm = model.fit(train_X, train_Y, epochs=120, batch_size=200, validation_split=0.2, verbose=0,
                             shuffle=False, callbacks=[EarlyStopping(monitor='val_loss', min_delta=0.002,
...
def model_stacked_lstm():  # to predict next n-day change
    ...
    model.compile(loss='mae', optimizer='adam')
    # fit network
    history_stacked_lstm = model.fit(train_X, train_Y, epochs=20, batch_size=60, validation_split=0.2, verbose=0,
...
def model_mixed():
    train_X, test_X, train_Y, test_Y = set_data()
    model = Sequential()
    model.add(GRU(80, return_sequences=True, input_shape=(train_X.shape[1], train_X.shape[2])))
    ...
    model.compile(loss='mae', optimizer='adam')
    # fit network
    history_mixed = model.fit(train_X, train_Y, epochs=120, batch_size=200, validation_split=0.2, verbose=0,
                              shuffle=False, callbacks=[EarlyStopping(monitor='val_loss', min_delta=0.002,
                                                                      patience=12, verbose=0, mode='min')])
...
def predict_result(model_name: str):  # import model_name
    ...
    train_X, test_X, train_Y, test_Y = set_data()
    y_predict = model.predict(test_X)    # (3337, 20)
    ...
    inv_y_predict = inv_y_test[:, 6: ]

    # calculate prediction changes of 20-day
    df = pd.DataFrame(inv_y_predict)
    df['change20_pred'] = df.apply(get_change20_pred, axis=1, result_type='expand')
    inv_y_predict = df['change20_pred'].values.tolist()

    # get original test data
    data = pre_process()
    data_true = data.iloc[:-20, :]
    values = data_true.values
    sep = int(0.9*len(values))
    test_true = values[sep: , :]
    inv_y_true = test_true[: , -1]
...
if __name__ == '__main__':
    model_mixed()
    show_loss(model_mixed)
    single_test('mixed')
    # mul_test(model_gru, 'gru')

    # adjust_params()

    # true, pred = predict_result('lstm')
    # print(len(true))